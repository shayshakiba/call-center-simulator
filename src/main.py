...
class Operator(Thread):
    # how long an operator has worked (since their last extended break)
    work_duration = 0
    ...
    def handle_caller(self, caller):
        ...
        # caller is sent to fill the survey
        caller.start()
    ...
    def rest(self):
        ...
        print(
            f'{self.name} returned after {round(break_duration, 1)} second(s).')
